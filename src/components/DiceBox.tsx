...
export const DiceBox = (props:DiceBoxProps) => {
  ...
  const [backLeftRef] = useBox(() => ({
    ...
    position: [-boxBottomSize*sqrt2/4, boxBottomSize / 2, boxBottomSize*sqrt2/4],
    ...props
  }))
  ...
  return (
    <>
      ...
      <mesh ref={(bottomRef as Ref<Mesh<BufferGeometry, Material | Material[]>> | undefined)} receiveShadow>
        <planeGeometry args={[boxBottomSize, boxBottomSize]} />
        <meshStandardMaterial attach={'material'} color={'#fff'} side={DoubleSide}/>
      </mesh>
      <mesh ref={(ceilRef as Ref<Mesh<BufferGeometry, Material | Material[]>> | undefined)} receiveShadow>
        <planeGeometry args={[boxBottomSize, boxBottomSize]} />
        <meshStandardMaterial attach={'material'} color={'#fff'}/>
      </mesh>
    </>
